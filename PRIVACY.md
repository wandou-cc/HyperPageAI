...
The extension has declared access to HTTP and HTTPS pages so its selection tools, page operations, and floating panel remain available while HyperPage is enabled. It cannot run on Chrome internal pages. HyperPage does not enumerate unrelated existing tabs for page tasks: it controls only the tab where the task started and tabs created by that task, and it never closes the starting tab. It reads, captures, or sends page content only after the user invokes an AI action or starts a page task. The `storage` permission keeps settings, workflows, and execution history locally. The `clipboardWrite` permission writes selected text or AI results only after the user chooses a copy action and does not read the clipboard. The `contextMenus` permission provides the enable switch on the extension icon. Disabling HyperPage from that menu removes its page interface and controller. Website access can also be managed in Chrome's extension settings. Removing the extension deletes its local configuration through Chrome.
...
扩展声明访问 HTTP 和 HTTPS 网页，以便启用 HyperPage 时持续提供元素选择、页面操作和悬浮面板；Chrome 内部页面不在可运行范围内。HyperPage 不会为页面任务枚举无关的已有标签页，仅控制任务发起页和本次任务创建的标签页，也不会关闭任务发起页。只有用户主动执行 AI 操作或启动页面任务后，扩展才会读取、截取或发送网页内容。`storage` 权限用于在本地保存设置、固定流程和执行记录；`clipboardWrite` 权限仅在用户主动复制时写入所选文字或 AI 结果，不会读取剪贴板；`contextMenus` 权限用于在扩展图标上提供启用开关。从该菜单停用 HyperPage 后，网页界面和页面控制器都会移除。用户也可在 Chrome 扩展设置中管理网站访问权限；卸载扩展后，Chrome 会删除扩展的本地配置。
